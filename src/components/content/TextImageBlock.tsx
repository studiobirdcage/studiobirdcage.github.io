...
export default function TextImageBlock({ title, text, image }: TextImageBlockProps) {
  return (
    <section className="flex flex-col-reverse px-6 py-12 min-[850px]:flex-row min-[850px]:px-30 min-[850px]:py-30">
      ...
      {image && <div className="mb-8 flex flex-1 items-center justify-center min-[850px]:mb-0 min-[850px]:ml-30"><img className="w-full" src={image} alt={imageAltFromPath(image, title ? `${title} project image` : "Project image")} loading="lazy" /></div>}
    </section>
  );
}
