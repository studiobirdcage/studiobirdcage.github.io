...
function PostCard({ post, featured }: { post: Post; featured: boolean }) {
  ...
  return (
    <article
      ...
      className={`relative bg-white ${featured ? "w-full" : "min-[1150px]:flex-1 min-[1150px]:basis-0"}`}
    >
      <a
        className={`group relative block overflow-hidden bg-[#121212] ${featured ? "h-125" : "h-100"}`}
        href={href}
      >
        <img
      ...
      >
        <ul
          className={`mb-3.5 flex list-none justify-center p-0 font-bold tracking-[-.5px] uppercase ${featured ? "text-lg max-[1149px]:text-sm max-[500px]:text-xs" : "text-sm max-[500px]:text-xs"}`}
        >
          <li className="mr-1 bg-[#cdcccf] px-3 py-1.5 text-white max-[500px]:px-2 max-[500px]:py-1">
            <a href={`/journal?category=${post.categorySlug}`}>
              <span className="solid-icon mr-1.5">&#xf07c;</span>
              {post.category}
            </a>
          </li>
          <li className="bg-[#00fdc7] px-3 py-1.5 text-white max-[500px]:px-2 max-[500px]:py-1">
            <span className="regular-icon mr-1.5">&#xf073;</span>
            {post.date}
          </li>
        </ul>
        <h2
          className={`leading-none font-black tracking-[-1px] text-[#121212] uppercase transition hover:text-[#00fdc7] ${featured ? "text-[44px] max-[1149px]:text-[32px] max-[500px]:text-[28px]" : "text-[32px] max-[500px]:text-[28px]"}`}
        >
          <a href={href}>{post.title}</a>
        </h2>
        <span className="mx-auto my-6 block h-0.5 w-1/5 bg-[#00fdc7]" />
        <p
          className={`font-sans tracking-[1px] text-[#808080] ${featured ? "text-lg max-[500px]:text-base" : "text-base"}`}
        >
          {post.excerpt}
        </p>
        {!!post.tags?.length && (
