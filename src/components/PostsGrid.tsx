...
function PostCard({ post, featured }: { post: Post; featured: boolean }) {
  ...
  return (
    <article data-post-category={post.categorySlug} className="relative bg-white">
      <a className={`group relative block overflow-hidden bg-[#121212] ${featured ? "h-[500px]" : "h-[400px]"}`} href={href}>
      ...
      <div className={`relative z-10 mx-auto -mt-20 w-[85%] bg-[#f9f8f9] px-8 py-10 text-center shadow-[0_0_35px_rgba(0,0,0,.15)] ${featured ? "md:px-30" : "md:px-16"}`}>
        <ul className={`mb-[14px] flex list-none justify-center p-0 font-bold uppercase tracking-[-.5px] ${featured ? "text-lg max-[1149px]:text-sm max-[500px]:text-xs" : "text-sm max-[500px]:text-xs"}`}><li className="mr-1 bg-[#cdcccf] px-3 py-1.5 text-white max-[500px]:px-2 max-[500px]:py-1"><a href={`/journal?category=${post.categorySlug}`}><span className="solid-icon mr-1.5">&#xf07c;</span>{post.category}</a></li><li className="bg-[#00fdc7] px-3 py-1.5 text-white max-[500px]:px-2 max-[500px]:py-1"><span className="regular-icon mr-1.5">&#xf073;</span>{post.date}</li></ul>
